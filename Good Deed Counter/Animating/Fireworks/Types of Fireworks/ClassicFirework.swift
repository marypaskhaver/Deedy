...
class ClassicFirework: Firework {
    ...
    // Firework is split into quarters like so
    /*
     x     |     x
        x  |   x
           |
    ...
    // Can flip parts of firework over
    ...
    private func shuffledQuarters() -> [Quarter] {
        ...
        for _ in 0..<quarters.count {
            let index = Int(arc4random_uniform(UInt32(quarters.count)))
            shuffled.append(quarters[index])
            quarters.remove(at: index) 
        }
